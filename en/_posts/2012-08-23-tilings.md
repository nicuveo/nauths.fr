...
Of course, when facing something a bit tedious, the first thing that
comes to mind is: how can I cheat my way around that? A prime example of
that is the aforementioned square tiling. If one wishes to implement
using a squared grid, it is useless to store in memory the full (and
verbose) squares list. It is indeed far simpler and easier to just
abstract the square gird as a 2D matrix; each cell of the matrix
corresponds one of the game squared cells.

This abstraction is perfect, because the Cartesian coordinates of each
square can easily be deduced from its position in the matrix. For a
given square size <tt>n</tt>, the cell at row <tt>y</tt> and column
<tt>x</tt> would correspond to:
